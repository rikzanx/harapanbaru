          ...
          <div class="col-12">
            <div class="card">
              <div class="card-header">
                <a href="{{ route('transaction.create') }}" class="btn btn-success"><span class="fas fa-plus"></span> tambah transaksi</a>
              </div>
              <!-- /.card-header -->
              <div class="card-body">
                <h3>Jumlah Saldo: @rupiahonly($wallet->balance)</h3>
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <tbody>
                    @foreach ($wallet->transactions as $item)
                    <tr>
                        <td>{{ $loop->index+1 }}</td>
                        <td>@rupiahonly($item->amount)</td>
                        {{-- <td>{{ $item->tipe }}</td> --}}
                        <td>
                          <div class="d-inline-flex mb-3 px-2 py-1 <?php echo ($item->tipe == "credit")?"bg-success":"bg-danger"; ?>" role="alert">
                            <?php echo ($item->tipe == "credit")?"Pemasukan":"Pengeluaran"; ?>
                          </div>
                        </td>
...
<div class="modal fade" id="modal-default">
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <form action="{{ route('wallet.destroy', ':id') }}" method="POST" class="delete-form">
            @csrf
            @method('DELETE')
